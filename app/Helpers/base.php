...
if (!function_exists('uploadFile')) {
    function uploadFile($path, $file, $file_exists = null)
    {
        $image = $file;
        $filename =  uniqid() . time() . '.' . $image->getClientOriginalExtension();
        $location = $path;
        $uploaded_path = $location . $filename;
        if (!blank($file_exists)) {
            \Storage::disk('public')->delete($file_exists);
        }
        $image->storeAs($location, $filename, 'public');
